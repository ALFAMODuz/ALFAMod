...
if [ ! -n "$BASH" ]; then
	...
	bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/ALFAMODuz/ALFAmod/raw/master/install.sh) '"$*"
	exit $?
fi
...
banner() {
	...
	clear
	printf "\n\e[1;35;47m                         \e[0m"
printf "\n\e[1;35;47m ALFAMod🕸️\e[0m"
printf "\n\e[1;35;47m                         \e[0m"
printf "\n\n\e[3;34;40m ALFAMod🕸️ - FTGning Avlodi \e[0m\n\n"
...
banner
printf '%s\n' "- Yuklash 5dan 15 daqiqagacha vaqt oladi"
...
if [ -d "alfamod/friendly-telegram" ]; then
	cd alfamod || {
...
${SUDO_CMD}rm -rf alfamod
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/ALFAMODuz/ALFAMod|| {
